...
class State(Dict):
    news_content: str
    retrieved_documents: str  # Added for retrieved documents
    summary: str
    correctness: str
    category: str
    sentiment: str
...
# RAG Function to fetch and return relevant documents
def retrieve_documents(query: str) -> str:
    # Here you would implement your retrieval logic.
    # For simplicity, let's assume we fetch relevant articles from the News API.
    articles = fetch_news_articles(query)
    return "\n\n".join(article["content"] for article in articles if article["content"])

# Node Functions with RAG Integration
def summarize(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Summarize the following news content along with relevant documents: {news_content}\n\n{retrieved_documents}"
    )
    chain = prompt | groq_instance
    summary = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"summary": summary}

def check_correctness(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Check the correctness of the following news content and the provided documents: {news_content}\n\n{retrieved_documents}"
    )
    chain = prompt | groq_instance
    correctness = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"correctness": correctness}

def categorize(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Categorize the following news article and relevant documents into Politics, Sports, Technology, or General. Article: {news_content}\n\n{retrieved_documents}"
    )
    chain = prompt | ChatGroq(temperature=0)
    category = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"category": category}

def analyze_sentiment(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Analyze the sentiment of the following news content and relevant documents: {news_content}\n\n{retrieved_documents}. "
        "Respond with either 'Positive', 'Neutral', or 'Negative'."
    )
    chain = prompt | ChatGroq(temperature=0)
    sentiment = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"sentiment": sentiment}

def detect_bias(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Detect any potential bias in the following news article and provided documents: {news_content}\n\n{retrieved_documents}."
    )
    chain = prompt | ChatGroq(temperature=0)
    bias = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"bias": bias}

def compute_reliability(state: State) -> State:
    prompt = ChatPromptTemplate.from_template(
        "Based on cross-referencing sources, provide a reliability score (0-100) for the following article and documents: {news_content}\n\n{retrieved_documents}"
    )
    chain = prompt | ChatGroq(temperature=0)
    score = chain.invoke({"news_content": state["news_content"], "retrieved_documents": state["retrieved_documents"]}).content
    return {"reliability_score": str(score)}

# Create and Configure the Graph
news_workflow = StateGraph(State)
news_workflow.add_node("retrieve_documents", retrieve_documents)
news_workflow.add_node("summarize", summarize)
news_workflow.add_node("check_correctness", check_correctness)
news_workflow.add_node("categorize", categorize)
news_workflow.add_node("analyze_sentiment", analyze_sentiment)
...
# Define the workflow connections
news_workflow.add_edge("retrieve_documents", "summarize")
news_workflow.add_edge("summarize", "check_correctness")
news_workflow.add_edge("check_correctness", "categorize")
news_workflow.add_edge("categorize", "analyze_sentiment")
news_workflow.add_edge("analyze_sentiment", "detect_bias")
...
# Set entry point
news_workflow.set_entry_point("retrieve_documents")
...
def run_news_agent(news_content: str) -> Dict[str, str]:
    """Process news content through the LangGraph workflow and return the results."""
    retrieved_documents = retrieve_documents(news_content)  # Fetch relevant documents
    results = news_app.invoke({"news_content": news_content, "retrieved_documents": retrieved_documents})
    return {
        "summary": results["summary"],
        "correctness": results["correctness"],
        "category": results["category"],
